...
unsigned int AudioManager::master_volume = 50;
...
void AudioManager::InitializeAudioSources()
{
	...
	audio_go = App->scene->GetGameObjectByName("SequenceController");
	if (audio_go != nullptr)
	{
		sequence_controller = static_cast<ComponentAudioSource*>(audio_go->GetComponent(Component::ComponentType::AUDIO_SOURCE));
	}
}
