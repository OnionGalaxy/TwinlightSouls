...
NewMushdoomBall* NewMushdoomBallDLL()
{
	NewMushdoomBall* instance = new NewMushdoomBall();
	return instance;
}

NewMushdoomBall::NewMushdoomBall()
{
	
}

void NewMushdoomBall::Awake()
{
	//mushdoom = static_cast<Mushdoom*>(owner->parent->GetComponentScript("Mushdoom")->script);
	mushdoom = static_cast<NewMushdoom*>(owner->parent->GetComponentScript("NewMushdoom")->script);
	collider = static_cast<ComponentCollider*>(owner->children[1]->GetComponent(Component::ComponentType::COLLIDER));
}

void NewMushdoomBall::Start()
{
	ResetBall();
}

void NewMushdoomBall::Update()
{
...
void NewMushdoomBall::OnInspector(ImGuiContext* context)
{
...
void NewMushdoomBall::ResetBall()
{
...
bool NewMushdoomBall::PlayerHit() const
{
...
float3 NewMushdoomBall::Parabola(const float3& start, const float3& end, float angle, float height, float t) const
{
