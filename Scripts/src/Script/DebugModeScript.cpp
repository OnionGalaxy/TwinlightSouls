...
void DebugModeScript::Awake()
{
	...
	ally_interaction_script = static_cast<AllyInteractionsScript*>(App->scene->GetGameObjectByName("AllyController")->GetComponentScript("AllyInteractionsScript")->script);
...
{
	ally_interaction_script->tutorial_request = false;
	ally_interaction_script->TransformIntoLightBall();
...
void DebugModeScript::SpawnEnemies()
{
	if(spawn_mushdoom)
	{
		enemy_manager->SpawnEnemy(EnemyType::MUSHDOOM, player_1->player_go->transform.GetGlobalTranslation() + spawn_offset, false, spawn_invincible);
	...
	if (spawn_biter)
	{
		enemy_manager->SpawnEnemy(EnemyType::BITER, player_1->player_go->transform.GetGlobalTranslation() + spawn_offset, false);
		spawn_biter = false;
		spawn_enemy = false;
	}
}
...
void DebugModeScript::DrawAudioOptions()
{
	if (ImGui::CollapsingHeader(ICON_FA_VOLUME_UP " Audio Options"))
	{
		if (ImGui::Button("-"))
		{
			if (audio_manager->master_volume > 0)
			{
		...
		ImGui::SameLine();
		if (ImGui::DragInt("Volume", (int*)&audio_manager->master_volume, 0, 100))
		{
			App->audio->SetRTPCValue("master_volume", audio_manager->master_volume);
		}
		ImGui::SameLine();
		if (ImGui::Button("+"))
		{
			if (audio_manager->master_volume < 100)
			{
				++audio_manager->master_volume;
				App->audio->SetRTPCValue("master_volume", audio_manager->master_volume);
			}
		...
		{
			App->audio->ResetRTPCValues("master_volume");
			audio_manager->master_volume = 50;
		}
	}
}
