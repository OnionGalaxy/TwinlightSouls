...
{
	float damage = attack_damage * world_manager->GetEnemyDamageMultiplier();
	current_target->TakeDamage(damage, current_target->hit_effect);
}

void EnemyController::Attack(float damage)	
{	
	float result_damage = damage * world_manager->GetEnemyDamageMultiplier();
	current_target->TakeDamage(result_damage, current_target->hit_effect);
}
