...
public:
	...
	void Revive();
	void SetIsInsideTrigger(bool is_inside_trigger) { this->is_inside_trigger = is_inside_trigger;}

	inline void OrbGettedByPlayer() { ++orbs_recollected; }
	inline unsigned int GetOrbsRecollected() const { return orbs_recollected; }
	inline float GetDamageReceived() const { return damage_received; }
	inline void EnemyKilledByPlayer() { ++enemies_killed; }
	inline unsigned int GetEnemiesKilled() { return enemies_killed; }
...
public:
	...
	ComponentParticleSystem* powerup_attack_VFX = nullptr;
	ComponentParticleSystem* powerup_invul_VFX = nullptr;
	...
	//POWERUPS
	float time_remaining_attack_powerup = 0.f;
	float attack_powerup_bonus = 0.0f;
	float time_remaining_invulnerable_powerup = 0.f;
...
private:
	...
	float total_health_with_difficulty = 400.f;
	...
	//Statistics
	unsigned int orbs_recollected = 0;
	float damage_received = 0.f;
	unsigned int enemies_killed = 0;

	GameObject* powerUpsPlayer1 = nullptr;
	GameObject* powerUpsPlayer2 = nullptr;
	bool jump_after_pause = true;
