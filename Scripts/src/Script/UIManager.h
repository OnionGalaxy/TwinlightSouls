...
public:
	...
	void EnableHUD(bool enable, bool multiplayer);
	void DisableComboCounter();
	...
	void HideAllPlayersUI(bool multiplayer) const;

	void UpdateShieldlUI(unsigned int player, float durability) const;
	void ShowShieldBar(bool show, unsigned int player) const;

	void ShowSkipButton();
	void HideSkipButton() const;
	GameObject* SelectSkipButton() const;
	bool IsSkipButtonActive() const;

	bool IsOnCredits() const;
	void ActiveCredits();
	GameObject* GetPlayerHUD(int player);
...
private:
	...
	ShieldBar* player1_shield_ui = nullptr;

	GameObject* player2_shield_ui_game_object = nullptr;
	ShieldBar* player2_shield_ui = nullptr;
	...
	bool on_credits = false;

	GameObject* skip_button_keyboard = nullptr;
	GameObject* skip_button_controller = nullptr;
	GameObject* selected_skip_button = nullptr;
