...
NewMushdoom* NewMushdoomDLL()
{
	NewMushdoom* instance = new NewMushdoom();
	return instance;
}

NewMushdoom::NewMushdoom()
{
	current_state = new NewMushdoomState();
	previous_state = new NewMushdoomState();
	idle_state = new NewMushdoomStateIdle(this);
	scream_state = new NewMushdoomStateScream(this);
	pursue_state = new NewMushdoomStatePursue(this);
	attack_state = new NewMushdoomStateAttack(this);
	die_state = new NewMushdoomStateDie(this);
	hit_state = new NewMushdoomStateHit(this);
	throw_state = new NewMushdoomStateThrow(this);
}

// Use this for initialization before Start()
void NewMushdoom::Awake()
{
	...
	for (const auto& ch : owner->children)
	{
		...
		{
			mushdoom_ball = static_cast<NewMushdoomBall*>(ch->GetComponentScript("NewMushdoomBall")->script);
			break;
		}
	}
	...
	horizontal_ray_detector_middle = new NewRayHit();
	horizontal_ray_detector_knee = new NewRayHit();

	main_vertical_ray_detector = new NewRayHit();
	border_vertical_ray_detector = new NewRayHit();
	main_diagonal_ray_detector = new NewRayHit();
}

// Use this for initialization
void NewMushdoom::Start()
{
...
void NewMushdoom::OnInspector(ImGuiContext* context)
{
	EnemyController::OnInspector(context);
}

void NewMushdoom::SetProperties(EnemyController* original_enemy)
{
	EnemyController::SetProperties(original_enemy);

	const NewMushdoom* original_mushdoom = (NewMushdoom*)original_enemy;
}

void NewMushdoom::KnockBack()
{
...
void NewMushdoom::TakeDamage(float damage, const float3& damage_source_position, float knockback)
{
...
void NewMushdoom::HitSFX(NodeID attack)
{
	switch (attack)
	{
...
bool NewMushdoom::DetectCollisions(int collision_direction)
{
	float3 current_position = owner->transform.GetGlobalTranslation();
	...
	else if (collision_direction == 2) //Rotation
	{
		...
		NewRayHit* rotation_ray = new NewRayHit();
...
bool NewMushdoom::IsColliding(NewRayHit* collision, const LineSegment& ray, bool only_mesh)
{
	bool collided = false;
...
bool NewMushdoom::EnemyInFront(const float3& enemy_position) const
{
...
void NewMushdoom::Move()
{
	...
	else
	{
		...
		ComputeHorizontalVelocity(direction, move_speed);
		if (has_ended_spawn)
		{
...
void NewMushdoom::ComputeHorizontalVelocity(float3 direction, float speed)
{
...
void NewMushdoom::ComputeVerticalVelocity(float3 direction, float speed, bool apply_gravity)
{
...
void NewMushdoom::SetNewPosition(const float3 & new_position) const
{
...
void NewMushdoom::ApplyVelocity()
{
	...
	if (!has_ended_spawn && distance <= enemy_manager->spawn_distance)
	{
...
void NewMushdoom::RotateTowardsPlayer() const
{
...
void NewMushdoom::SpawnJump() const
{
...
void NewMushdoom::ResetMovement()
{
...
void NewMushdoom::HandleAttackZones(float3& new_position)
{
...
void NewMushdoom::LimitMovementAtDistance(float3& new_position, float max_distance)
{	
	...
	ComputeHorizontalVelocity((new_position - position).Normalized(), move_speed);
	new_position = owner->transform.GetGlobalTranslation() + velocity * App->time->delta_time * 0.001f;
}

void NewMushdoom::RotateAtDistance(float3& new_position, float max_distance, float angle)
{
	float3 gronund_pos = float3(new_position.x - target.x, 0.0f, new_position.z - target.z);
	float3 gronund_pos_norm = gronund_pos.Normalized();
...
bool NewMushdoom::IsOutsideOfRange() const
{
	if ((attack_slot > -1 || shoot_slot > -1  || seek_slot > -1) && distance <= enemy_manager->spawn_distance)
	{
		return false;
	}
	return true;
...
void NewMushdoom::AvoidEnemies(float3& new_position, std::vector<EnemyController*>* enemies)
{
	float min_space = 3.f;
	...
	if (previous_enemy)
	{
		...
		if (space_between < min_space)
		{
			...
			new_velocity = new_velocity.ScaledToLength(space_between - min_space);
...
bool NewMushdoom::IsNextTo(float3& initial, float3& goal, bool is_left)
{
