...
void AllyInteractionsScript::Awake()
{
	...
	controller_helper_ui = App->scene->GetGameObjectByName("AllyControllerHelper");
	keyboard_helper_ui = App->scene->GetGameObjectByName("AllyKeyboardHelper");
	...
	GameObject* cinematic_camera_holder = App->scene->GetGameObjectByName("Cinematic Camera Holder");
	if (cinematic_camera_holder)
	{
		cinematic_camera = static_cast<CinematicCamera*>(cinematic_camera_holder->GetComponentScript("CinematicCamera")->script);
	}
	//lightball
...
{
	CheckShieldTutorial();
	if (cinematic_camera && cinematic_camera->IsPlayingCinematic())
	{
		return;
	}
...
void AllyInteractionsScript::AllyLightBallUpdate()
{
	if (dialogue_manager->IsOnDialogue())
	{
		...
		Quat rotation = Quat::RotateAxisAngle(light_ball->owner->transform.GetUpVector(), adjusted_time);

		const float3& center = lightBall_translation_reference;
		float3 position = light_ball->owner->transform.GetTranslation();

		float3 direcction_vector = position - center;

		direcction_vector = rotation * direcction_vector;
		position = direcction_vector + center;
...
void AllyInteractionsScript::AllyHumanoidUpdate()
{
	...
	if (!dialogue_manager->IsOnDialogue() && player_is_inside_interaction_range && can_start_next_dialogue)
	{
		...
		bool player_input = (!world->on_pause && input_manager->GetGameInputUp("Jump", static_cast<PlayerID>(player_1->player_controller->player)));
		if (player_input)
		{
...
void AllyInteractionsScript::ActivateLightBallDialogue(DialogueID dialogue)
{
	if (!dialogue_manager->IsOnDialogue() )
	{
		SetLightballPositionReference();
		dialogue_manager->StartDialogue(dialogue, [this](DialogueFeeling feeling) { LightBallModeCallback(feeling); });
	}
}

void AllyInteractionsScript::SetLightballPositionReference()
{
	lightBall_translation_reference = light_ball->current_player->transform.GetGlobalTranslation();
	lightBall_translation_reference.y = light_ball->current_player_hand->transform.GetGlobalTranslation().y;
}
...
void AllyInteractionsScript::TutorialSkippablePopUp()
{
	EnableAllyOnConversation(true);
}

bool AllyInteractionsScript::GetIsTeleported() const
{
	return is_ally_teleported;
}

 TutorialHelper* AllyInteractionsScript::GetTutorialHelper() const
{
	return tutorial_helper.get();
}

InputManager* AllyInteractionsScript::GetCurrentInput() const
{
	return input_manager;
}

void AllyInteractionsScript::SetIsTeleported(bool is_ally_teleported)
{
	this->is_ally_teleported = is_ally_teleported;
}