...
void UIManager::Awake()
{
	...
	if (!world->player1_choice)
	{
		player1_shield_ui = static_cast<ShieldBar*>(player1_shield_ui_game_object->GetComponentScript("ShieldBar")->script);
		player2_shield_ui = static_cast<ShieldBar*>(player2_shield_ui_game_object->GetComponentScript("ShieldBar")->script);
	}
	else 
	{
		player2_shield_ui = static_cast<ShieldBar*>(player1_shield_ui_game_object->GetComponentScript("ShieldBar")->script);
		player1_shield_ui = static_cast<ShieldBar*>(player2_shield_ui_game_object->GetComponentScript("ShieldBar")->script);
...
void UIManager::HideAllPlayersUI(bool multiplayer) const
{
	player1_light_ball_ui_game_object->parent->SetEnabled(false);
	if (multiplayer)
	{
		player2_light_ball_ui_game_object->parent->SetEnabled(false);
	}
}
...
void UIManager::UpdateShieldlUI(unsigned int player, float durability) const
{
	if (player == 0)
	{
		...
		if (!world->player1_choice) 
		{
			obj = App->scene->GetGameObjectByName("Player1");
		}
		else 
		{
			obj = App->scene->GetGameObjectByName("Player2");
		}
		
		player1_shield_ui->SetPlayerPosition(obj->transform.GetGlobalTranslation());
	}
	else
	{
		GameObject* obj;
		...
		player2_shield_ui->SetPlayerPosition(obj->transform.GetGlobalTranslation());
	}
}

void UIManager::ShowShieldBar(bool show, unsigned int player) const
{
	if (player == 0)
	{
		player1_shield_ui->active = show;
	}
	else
	{
		player2_shield_ui->active = show;
	}
...
void UIManager::EnableHUD(bool enable, bool multiplayer)
{
	player1_hud->SetEnabled(enable);
	lives_ui->owner->SetEnabled(enable);
	if (multiplayer)
	{
		player2_hud->SetEnabled(enable);
	}
}

void UIManager::DisableComboCounter()
{
	combo_counter_indicator->owner->SetEnabled(false);
}
...
GameObject* UIManager::GetPlayerHUD(int player)
{
	if (player == 1)
	{
		return player1_hud;
...
void UIManager::InitPublicGameObjects()
{
	...
	public_gameobjects.push_back(&skip_button_keyboard);
	variable_names.push_back(GET_VARIABLE_NAME(skip_button_keyboard));

	public_gameobjects.push_back(&skip_button_controller);
	variable_names.push_back(GET_VARIABLE_NAME(skip_button_controller));
...
void UIManager::ShowSkipButton()
{
	selected_skip_button = SelectSkipButton();
	selected_skip_button->SetEnabled(true);
}

void UIManager::HideSkipButton() const
{
	if (selected_skip_button != nullptr)
	{
		selected_skip_button->SetEnabled(false);
	}
}

GameObject* UIManager::SelectSkipButton() const
{
	if (input_manager->total_game_controllers == 0)
	{
		skip_button_controller->SetEnabled(false);
		return skip_button_keyboard;
	}
	else
	{
		skip_button_keyboard->SetEnabled(false);
		return skip_button_controller;
	}
}

bool UIManager::IsSkipButtonActive() const
{
	if (selected_skip_button != nullptr)
	{
		return selected_skip_button->IsEnabled();
	}
	return false;
}

bool UIManager::IsOnCredits() const
{
	return on_credits;
}

void UIManager::ActiveCredits()
{
	on_credits = true;
}
