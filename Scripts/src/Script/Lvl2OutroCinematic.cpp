...
namespace
{
	...
	const std::string LIGHTBALL_FIRST_DIALOGUE = "Good! Let's get deeper into the forest!";
...
Lvl2OutroCinematic::Lvl2OutroCinematic(ModuleScene& scene, ModuleTime& time) : Cinematic(scene, time)
{
	...
	event_manager->outro_lvl2_finished = false;
}

// Update is called once per frame
void Lvl2OutroCinematic::Update()
{
	lvl2_outro_timeline.Update(time_module.delta_time);
}


void Lvl2OutroCinematic::Play()
{
	CreateLvl2OutroCinematic();
	lvl2_outro_timeline.Play();
	EnableTimeline(true);
}

void Lvl2OutroCinematic::Stop()
{
	lvl2_outro_timeline.Stop();
}

bool Lvl2OutroCinematic::IsPlaying() const
{
	return lvl2_outro_timeline.IsPlaying();
}

void Lvl2OutroCinematic::CreateLvl2OutroCinematic()
{
	lvl2_outro_timeline.tracks.clear();
	...
	lvl2_outro_timeline.tracks.emplace_back(camera_track);


	if (!world_manager->multiplayer)
	{
		p1_pos = scene_module.GetGameObjectByName("single_player_pos");
		p1_to_door = scene_module.GetGameObjectByName("single_player_pos_2");
	...
	else
	{
		...
		p1_to_door = scene_module.GetGameObjectByName("p1_pos_2");
		p2_to_door = scene_module.GetGameObjectByName("p2_pos_2");
		...
		lvl2_outro_timeline.tracks.emplace_back(player2_track_animation);
		...
		player2_track->AddKeyframe(8.2f, p2_to_door->transform.GetGlobalModelMatrix());
		player2_track->AddKeyframe(10.2f, p2_final->transform.GetGlobalModelMatrix());

		lvl2_outro_timeline.tracks.emplace_back(player2_track);
	}
	...
	lvl2_outro_timeline.tracks.emplace_back(player1_track_animation);
	...
	player1_track->AddKeyframe(8.2f, p1_to_door->transform.GetGlobalModelMatrix());
	player1_track->AddKeyframe(10.2f, p1_final->transform.GetGlobalModelMatrix());

	lvl2_outro_timeline.tracks.emplace_back(player1_track);
	...
	lvl2_outro_timeline.tracks.emplace_back(lb_track);
	...
	lvl2_outro_timeline.tracks.emplace_back(custom_track);


}

void Lvl2OutroCinematic::Trigger(const std::string & trigger_name, const void* data)
{
