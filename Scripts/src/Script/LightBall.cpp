...
namespace
{
	const float4 small_final_agressive_color = float4(1.f, 1.f, 1.f, 1.f);
	const float4 interior_final_agressive_color = float4(10.f/255.f, 150.f/255.f, 10.f/255.f, 255.f/ 255.f);
	const float4 exterior_final_agressive_color = float4(31.f/255.f, 10.f/255.f, 56.f/255.f, 0.5f);
}
...
void LightBall::Awake()
{
	...
	//Interior
	interior_initial_color = interior_texture_LB->color;
	//interior_initial_color_emissive = interior_texture_LB->color_emissive;

	//Exterior
	exterior_initial_color = exterior_texture_LB->color;
	//exterior_initial_color_emissive = exterior_texture_LB->color_emissive;

	//Small
	small_initial_color = small_texture_LB->color;
	small_initial_color_emissive = small_texture_LB->color_emissive;

	//Trail
	trail_initial_color = trail_light_ball_VFX->color;
	trail_initial_color_to_blend = trail_light_ball_VFX->color_to_blend;
}
...
void LightBall::Start()
{
	...
	player_1_hand = player_1->player_go->children[0]->children[0];
	player_2_hand = player_2->player_go->children[0]->children[0];
	...
	case LightBallState::ACTIVATED:
		...
		if (casted && current_player_controller->animation->IsOnState("ThrowingBall"))
		{
			...
			if (current_player_controller->animation->GetCurrentClipPercentatge() >= 0.6f)
			{
				...
				force_position = (current_player_hand->transform.GetGlobalTranslation()
					+ current_player_hand->transform.GetFrontVector() * cast_distance) 
					+ current_player->transform.GetFrontVector() * distance;
				frame_counter = 0.f;
...
{
	float damage_base = damage;
	if (current_player_controller->time_remaining_attack_powerup > 0)
	{
		damage_base *= current_player_controller->attack_powerup_bonus;
	}
	...
	for (auto& enemy : enemy_manager->enemies)
	{
		...
		{
			enemy->TakeDamage(damage_base, owner->transform.GetGlobalTranslation());
			enemy->HitLightBallVFX();
			enemies_hitted.push_back(enemy);
			combo_counter->AddHit(damage_base);
			if(enemy->GetCurrentHealth() < 0.1f)
			{
				current_player_controller->EnemyKilledByPlayer();
			}
		}
	}
	...
	if (is_boss)
	{
		...
		{
			boss->boss_controller->TakeDamage(damage_base, current_pos, position_hitted);
			boss_hitted = true;
		}
	}
}
...
void LightBall::SetActiveVFX(bool active) const
{
	...
	if (active)
	{
		trail_light_ball_VFX->duration = 450.0F;
	}
	else
	{
...
void LightBall::RestartBall()
{
	...
	interior_texture_LB->color = interior_initial_color;
	//interior_texture_LB->color_emissive = interior_initial_color_emissive;
	exterior_texture_LB->color = exterior_initial_color;
	//exterior_texture_LB->color_emissive = exterior_initial_color_emissive;
	small_texture_LB->color = small_initial_color;
	small_texture_LB->color_emissive = small_initial_color_emissive;
	trail_light_ball_VFX->color = trail_initial_color;
	trail_light_ball_VFX->color_to_blend = trail_initial_color_to_blend;
...
bool LightBall::GetCloserEnemy(float3& closer_position)
{
	closer_position = float3::zero;
	float enemy_distance = distance;
	...
	for (auto& enemy : enemy_manager->enemies)
	{
		...
		float3 position_with_offset = owner->transform.GetGlobalTranslation() - owner->transform.GetFrontVector();
		...
		if (forward_vector.Dot(direction) > angle_autoaim)
		{
			...
			if (current_enemy_distance < enemy_distance)
			{
				enemy_distance = current_enemy_distance;
...
void LightBall::AutoAimLightball()
{
	float3 closer_position = float3::zero;
	bool needs_autoaim = GetCloserEnemy(closer_position);

	if (needs_autoaim)
	{
		float3 direction_to_enemy = closer_position - owner->transform.GetGlobalTranslation();
		current_player_controller->player_movement->RotatePlayer(direction_to_enemy, smooth_autoaim_rotation);
	}
}

void LightBall::SetLightballColor() const
{
	float color_percentatge = 4.f * speed / 8.f;
	float4 interior_final_agressive_color_emissive = interior_final_agressive_color;
	interior_final_agressive_color_emissive.w = 1.f;
	float4 exterior_final_agressive_color_emissive = exterior_final_agressive_color;
	exterior_final_agressive_color_emissive.w = 1.f;
	interior_texture_LB->color = math::Lerp(interior_texture_LB->color, interior_final_agressive_color_emissive, min(color_percentatge, 1.f));
	//interior_texture_LB->color_emissive = math::Lerp(interior_texture_LB->color_emissive, interior_final_agressive_color, min(color_percentatge, 1.f));
	exterior_texture_LB->color = math::Lerp(exterior_texture_LB->color, exterior_final_agressive_color, min(color_percentatge, 1.f));
	//exterior_texture_LB->color_emissive = math::Lerp(exterior_texture_LB->color_emissive, exterior_final_agressive_color_emissive, min(color_percentatge, 1.f));
	small_texture_LB->color = math::Lerp(small_texture_LB->color, small_final_agressive_color, min(color_percentatge, 1.f));
	//small_texture_LB->color_emissive = math::Lerp(small_texture_LB->color, small_final_agressive_color, min(color_percentatge, 1.f));
	trail_light_ball_VFX->color = math::Lerp(exterior_texture_LB->color, exterior_final_agressive_color, min(color_percentatge, 1.f));
	trail_light_ball_VFX->color_to_blend = math::Lerp(interior_texture_LB->color_emissive, interior_final_agressive_color, min(color_percentatge, 1.f));
}
