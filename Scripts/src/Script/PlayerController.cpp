...
void PlayerController::Awake()
{
	...
	powerUpsPlayer1 = App->scene->GetGameObjectByName("Player 1 Powerups");
	powerUpsPlayer1->children[0]->SetEnabled(false);
	powerUpsPlayer1->children[1]->SetEnabled(false);
	
	powerUpsPlayer2 = App->scene->GetGameObjectByName("Player 2 Powerups");
	powerUpsPlayer2->children[0]->SetEnabled(false);
	powerUpsPlayer2->children[1]->SetEnabled(false);
...
void PlayerController::Start()
{
	...
	total_health_with_difficulty = total_health * world->GetHealthMultiplier();
	health_points = total_health_with_difficulty;
	
	if(player == WhichPlayer::PLAYER1)
	{
		other_player = world->GetPlayer2();
		other_player_id = 2;
		ui_manager->SetPlayer1Health(health_points / total_health_with_difficulty);
	}
	else
	{
		other_player = world->GetPlayer1();
		ui_manager->SetPlayer2Health(health_points / total_health_with_difficulty);
	}
	...
	if (jump_after_pause) {
		...
		case PlayerState::ALIVE:
			if (!is_reviving)
			{
				...
				light_shield->player_alive = true;
			}
		...
		case PlayerState::DYING:
			CheckDyingStatus();
			light_shield->player_alive = false;
...
{
	if (debug->is_player_invincible || debug->is_player_god_mode || is_dashing || spawning || time_remaining_invulnerable_powerup > 0)
	{
		return;
	}
...
{
	if(is_alive && health_points > 0.f)
	{
		health_points = math::Min(health_points + healed_value, total_health_with_difficulty);
		UpdateHealthBar();	
	}
}
...
void PlayerController::Die()
{
	...
	player_movement->enemy_manager->ClearAllSlots(static_cast<unsigned int>(player));
	is_alive = false;
	...
	if (has_fallen)
	{
		owner->transform.SetGlobalMatrixTranslation(falling_spawn_point);
	}
}
...
void PlayerController::Revive()
{
	...
	dead_indicator->SetEnabled(false);
	animation->SetBool("is_alive", true);
	health_points = total_health_with_difficulty /4;
	revive_bar_go->SetEnabled(false);
	...
	{
		ui_manager->SetPlayer1Health(health_points / total_health_with_difficulty);
		ui_manager->SetPlayer2Health(other_player->player_controller->health_points / total_health_with_difficulty);
	}
	else
	{
		ui_manager->SetPlayer2Health(health_points / total_health_with_difficulty);
		ui_manager->SetPlayer1Health(other_player->player_controller->health_points / total_health_with_difficulty);
	}
...
void PlayerController::CheckLightShieldStatus() const
{
	...
	{
		DialogueManager* dialogue_manager = static_cast<DialogueManager*>(App->scene->GetGameObjectByName("Dialogue Manager")->GetComponentScript("DialogueManager")->script);
		if (dialogue_manager->IsOnDialogue() || is_inside_trigger)
		{
...
void PlayerController::MoveAndAttack()
{
	...
	if (!is_attacking)
	{
		player_movement->Move(static_cast<uint32_t>(player), can_move);
	}
...
void PlayerController::CheckDeadBehaviour()
{
	...
	sphere_mesh_render->active = false;
	player_movement->Move(static_cast<uint32_t>(player), true);
...
bool PlayerController::CheckDashBehaviour()
{
	if (is_dashing)
	{
		...
		{
			dash_trail_VFX->duration = 300.0f;
			player_movement->MoveIntoDirection(dash_direction, dash_speed);
...
void PlayerController::InitPowerups()
{
	time_remaining_attack_powerup = 0;
	time_remaining_invulnerable_powerup = 0;

	powerup_attack_VFX->Stop();
	powerup_invul_VFX->Stop();
}

void PlayerController::UpdatePowerups()
{
	float delta_time = App->time->delta_time / 1000.f;

	if (player == WhichPlayer::PLAYER1) {
		powerUpsPlayer1->children[0]->SetEnabled(time_remaining_attack_powerup != 0);
		powerUpsPlayer1->children[1]->SetEnabled(time_remaining_invulnerable_powerup != 0);
	}
	else {
		powerUpsPlayer2->children[0]->SetEnabled(time_remaining_attack_powerup != 0);
		powerUpsPlayer2->children[1]->SetEnabled(time_remaining_invulnerable_powerup != 0);
	}

	if (time_remaining_attack_powerup != 0)
	{

		if (!powerup_attack_VFX->HasParticlesAlive())
		{
			powerup_attack_VFX->Play();
		}
		time_remaining_attack_powerup -= delta_time;
		if (time_remaining_attack_powerup < 0)
		{
			time_remaining_attack_powerup = 0;
			powerup_attack_VFX->Stop();
	...
	if (time_remaining_invulnerable_powerup != 0)
	{
		if (!powerup_invul_VFX->HasParticlesAlive())
		{
			powerup_invul_VFX->Play();
		}
		time_remaining_invulnerable_powerup -= delta_time;
		if (time_remaining_invulnerable_powerup < 0)
		{
			time_remaining_invulnerable_powerup = 0;
			powerup_invul_VFX->Stop();
...
void PlayerController::InitVFX()
{
	...
	for (const auto& go : owner->children)
	{
		...
		if (go->name == "PowerupAttackVFX")
		{
			powerup_attack_VFX = static_cast<ComponentParticleSystem*>(go->GetComponent(Component::ComponentType::PARTICLE_SYSTEM));
			powerup_attack_VFX->Stop();
		}
		if (go->name == "PowerupInvulnerabilityVFX")
		{
			powerup_invul_VFX = static_cast<ComponentParticleSystem*>(go->GetComponent(Component::ComponentType::PARTICLE_SYSTEM));
			powerup_invul_VFX->Stop();
		}
	}
}