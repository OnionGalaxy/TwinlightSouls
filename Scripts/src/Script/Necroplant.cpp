...
void Necroplant::Awake()
{
	...
	initial_position = this->owner->transform.GetGlobalTranslation();
...
void Necroplant::AimingState()
{
	...
	if (!burrowed_recently && PlayerInRange(danger_radius) && necro_spot != nullptr && !is_static)
	{
...
void Necroplant::ResetEnemy()
{
	...
	{
		owner->transform.SetGlobalMatrixTranslation(initial_position);
	}
...
void Necroplant::Save(Config& config) const
{
...
void Necroplant::Load(const Config &config)
{
