...
void BossController::Start()
{
	...
	InitRocks();
	InitEffects();
	SetAvalancheSpeed(750.f, 1100.f);
...
void BossController::Update()
{
	...
	if (is_alive && !world_manager->on_pause)
	{
		current_stage->Update(App->time->delta_time);

		//Update boss health bar
	}
	else if(boss_dissolver && !boss_dissolver->IsDissolving())
	{
		this->owner->SetEnabled(false);
		boss_dissolver = nullptr;
	}
...
void BossController::OnInspector(ImGuiContext* context)
{
	...
	{
		bool aux_bool = crystal.spawned;
		ImGui::Checkbox("Spawned", &aux_bool);
		ImGui::SameLine();
		float aux_float = crystal.remaining_time_alive;
		ImGui::DragFloat("Remaining time", &aux_float);
	}
...
{
	stages.push(new BossStagePhaseFourth(this));
...
{
	//static_cast<ComponentAudioSource*>(App->scene->GetGameObjectByName("TimelineManager")->GetComponent(Component::ComponentType::AUDIO_SOURCE))->PlayEvent("Play_BossIntroMusic");
}

void BossController::StopMusic() const
{
	static_cast<ComponentAudioSource*>(App->scene->GetGameObjectByName("TimelineManager")->GetComponent(Component::ComponentType::AUDIO_SOURCE))->PlayEvent("Stop_BossIntroMusic");
}
...
{
	GameObject* crystals_go = App->scene->GetGameObjectByName("Crystals");
	for(auto go : crystals_go->children)
	{
		go->SetEnabled(false);
...
{
	GameObject* rocks_go = App->scene->GetGameObjectByName("Rocks");
	...
	for (auto& zone : vulnerable_zones)
	{
		if (zone->active) 
		{
			if (zone->zone_go->aabb.bounding_box.Contains(ball_position))
			{
				...
				current_damage_position = hit_position_object;
				return true;
			}
		}
	}
...
bool BossController::HitByPlayer(const float3& player_position, const float3& player_front_vector, const float range, float3& hit_position_object) const
{
	...
	LineSegment ray_line_collision_detector(collision_detector_position, collision_detector_position + player_front_vector * range / player_range_factor);
...
void BossController::TakeDamage(float damage, const float3& damage_dealer_position, const float3& hitted_position_object)
{
	...
	if (current_health <= 0.f) 
	{
		...
		light_orbs_manager->SpawnOrbs(current_damage_position, 30);
		light_orbs_manager->SpawnPowerup(current_damage_position);
	}
	else if(current_health < next_health_stage && !stages.empty())
	{
		...
		//Spawn orbs
		light_orbs_manager->SpawnOrbs(current_damage_position, 8);
		light_orbs_manager->SpawnPowerup(current_damage_position);
	}
...
void BossController::SetBossShadowCaster(bool caster) const
{

	static_cast<ComponentMeshRenderer*>(owner->children[0]->GetComponent(Component::ComponentType::MESH_RENDERER))->SetShadowCaster(caster);
}

void BossController::HideHealthBar()
{
	boss_health_bar->owner->SetEnabled(false);
}

void BossController::DisableBoss()
{
	owner->SetEnabled(false);
}
...
void BossController::BossKilled()
{
	if (!event_manager->boss_killed)
	{
		...
		boss_dissolver = static_cast<Dissolver*>(owner->GetComponentScript("Dissolver")->script);
		boss_dissolver->EnableInvertedDissolver(true);
		boss_dissolver->SetDissolvingTime(2000.f);
		boss_dissolver->SetStartingDissolveProgress(0.0f);
		boss_dissolver->Dissolve();
		is_alive = false;
		SetBossShadowCaster(false);
	}
}
...
void BossController::SetAvalancheSpeed(float min_time, float max_time)
{
	min_remaining_rock_time_alive = min_time;
	max_remaining_rock_time_alive = max_time;
}
