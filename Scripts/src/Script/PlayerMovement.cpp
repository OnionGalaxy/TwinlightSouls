...
void PlayerMovement::Awake()
{
	...
	enemy_manager = static_cast<EnemyManager*>(enemy_go->GetComponentScript("EnemyManager")->script);
...
void PlayerMovement::Move(int player, bool can_move)
{
	...
	if (can_move && input_manager->GetGameInputDown("Jump", player_id) && !input_manager->GetGameInputDown("Dash", player_id))
	{
...
void PlayerMovement::SlowPlayer(float percentatge)
{
	...
	speed_horizontal = min(default_speed_horizontal - default_speed_horizontal * percentatge * 0.01f, speed_horizontal);
	if(percentatge == 0.f)
	{
		speed_horizontal = default_speed_horizontal;
	}
}
...
void PlayerMovement::Load(const Config& config)
{
	...
	speed_horizontal = config.GetFloat("HorizontalSpeed", default_speed_horizontal);
	acceleration = config.GetFloat("Acceleration", default_acceleration);
	deceleration = config.GetFloat("Deceleration", default_deceleration);
