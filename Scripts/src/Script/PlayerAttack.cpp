...
void PlayerAttack::Awake()
{
	...
	controller = static_cast<PlayerController*>(owner->GetComponentScript("PlayerController")->script);
...
{
	const ComponentScript* player_controller_go = owner->GetComponentScript("PlayerController");
	player_controller = static_cast<PlayerController*>(player_controller_go->script);

	const ComponentScript* player_movement_go = owner->GetComponentScript("PlayerMovement");
	player_movement = static_cast<PlayerMovement*>(player_movement_go->script);
...
bool PlayerAttack::Attack(uint32_t player)
{
	...
	{
		 MakeAttack();
		 App->engine_log->Log("MAKE ATTACK");
	}
	...
	else if(is_attacking && anim_percentatge <= back_to_idle_percentatge)
	{
		if(!current_attack->is_aerial && current_attack->speed > 0.f && animation->IsOnState(current_attack->animation_state))
		{
			...
			&& on_state && same_attack && anim_percentatge > current_attack->smash_anim_percentatge)
		{
...
void PlayerAttack::Hit(EnemyController* enemy)
{
	...
	if (player_controller->time_remaining_attack_powerup > 0)
	{
		current_damage_power *= player_controller->attack_powerup_bonus;
	}
	...
	{
		controller->EnemyKilledByPlayer();
	}
...
void PlayerAttack::CheckAttackInputs(int player)
{
	...
	if(input_manager->GetGameInputDown("Punch", static_cast<PlayerID>(player)))
	{
		if (!player_movement->IsGrounded())
		{
			InputCombo* input_combo = new InputCombo(Move::LIGHT_JUMP_ATTACK, time);
...
{
	bool female_character = world_manager->GetPlayer1()->player_go == owner && world_manager->player1_choice
		|| world_manager->GetPlayer2()->player_go == owner && !world_manager->player1_choice;
	...
	if (female_character)
	{
		App->engine_log->Log("Female attack");
		light_one = new LightOneFemale();
		light_two = new LightTwoFemale();
		light_three = new LightThreeFemale();
		heavy_one = new HeavyOneFemale();
		heavy_two = new HeavyTwoFemale();
		heavy_three = new HeavyThreeFemale();
		down_smash = new DownSmashFemale();
	}
	else
	{
		light_one = new LightOne();
		light_two = new LightTwo();
		light_three = new LightThree();
		heavy_one = new HeavyOne();
		heavy_two = new HeavyTwo();
		heavy_three = new HeavyThree();
		down_smash = new DownSmash();
	}
	...
	LightJumpAttack* light_jump_attack = new LightJumpAttack();
	attack_list.emplace_back(light_jump_attack);
}
...
bool PlayerAttack::EnemyOnRange(EnemyController* enemy) const
{
	...
	//For downtail smash
	else if(current_attack->is_smash)
	{
		return true;
	}
