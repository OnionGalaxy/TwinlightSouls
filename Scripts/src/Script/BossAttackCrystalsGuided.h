...
public:
	...
	inline void SetGuidedState(GuidedProjectilesState state) { guided_projectile_state = state; }
	inline void SetChargingFromGround(float time) { time_charging_from_ground = time; }
...
private:
	...
	float time_charging_from_ground = 400.f;
